...
SET @params = @params + '&cu='+@cu
SET @params = @params + '&uip='+@uip
...
select @res_status as status, @response as response
...
--	SET @params = @params + '&ic=u3eqds43'			--// Item code / SKU. //ÌÎÆÍÎ. Åñëè åñòü
	...
	SET @params = @params + '&cu='+@cu				--// Currency code.   //ÍÀÄÎ 
	SET @params = @params + '&uip='+@uip

	--exec dbo.US_WEB_GA_POST @params, @res_status out, @response out, @debug
	exec dbo.US_WEB_GA_GET @params, @res_status out, @response out, @debug
	select @res_status as status, @Reponse as response
end
else
