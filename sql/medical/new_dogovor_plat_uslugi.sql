...
select 
isnull(nomer_platnogo_dogovora,'')
,data126_id
...
begin
	select 'dopinfo data126 record exists'
end
else
begin
...
/*
2) Если поле договор пусто, то нужно создать новый номер договора и новое прикрепление с датами, старое закрыть.
3) Если поле договор заполнено, то нужно проверять год и даты прикреплений.
*/

if(len(@dog_num)>0)
	select 'dog number is '+@dog_num
else
begin
	declare @Value integer, @Template varchar(50)

	exec dbo.up_get_counter_value  @KeyName = 'pu_dog_num',  @Shift = 1,
		@Value = @dog_num output, @Template = @Template output

	set @dog_num= convert(varchar(4),datepart(yyyy,getdate()))+'-'+@dog_num
...
/*
проверить полис и номер договора
проверить date_to
если дата меньше, то нужен новый номер договора
*/
	select fm_clink_id,date_to,police
