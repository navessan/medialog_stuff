...
select
	@current_pat=131509		--текущий объединенный пациент
	,@new_pat_id=835174		--куда переносим записи в текущей базе
	,@backup_pat_id=619398	--удаленный пациент
...
/*
case 
when old.PATIENTS_ID=@backup_pat_id then old_pat.nom
...
--and old.PATIENTS_ID=@backup_pat_id
...
FROM fm_invoice
inner join FM_BILLDET_PAY on FM_BILLDET_PAY.fm_invoice_ID=fm_invoice.fm_invoice_ID
inner join FM_BILLDET on (FM_BILLDET_PAY.FM_BILLDET_ID=FM_BILLDET.FM_BILLDET_ID)
inner JOIN FM_BILL FM_BILL ON FM_BILL.FM_BILL_ID = FM_BILLDET.FM_BILL_ID
WHERE
FM_BILL.MOTCONSU_ID in (@MOTCONSU_ID)
