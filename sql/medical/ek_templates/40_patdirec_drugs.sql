...
--select @offset_PATDIREC_ID=MAX(patdirec_id) from PATDIREC

select @offset_PATDIREC_DRUGS_ID=MAX(PATDIREC_DRUGS_ID) from PATDIREC_DRUGS
...
--,PATDIREC.[PATDIREC_ID]
 @offset_PATDIREC_DRUGS_ID+ROW_NUMBER()over (order by PATDIREC.[PATDIREC_ID]) as [PATDIREC_DRUGS.PATDIREC_DRUGS_ID]
